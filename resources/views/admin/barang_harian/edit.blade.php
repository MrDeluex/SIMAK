        ...
        document.addEventListener("DOMContentLoaded", async function() {
            ...
            if (harian) {
                ...
                document.getElementById("staff_produksi_id").innerHTML = '<option value="" disabled>Pilih Karyawan</option>' +
                    data.map(staff => `<option value="${harian.data.staff_produksi.id}" ${harian.data.staff_produksi.id == harian.data.staff_produksi.id ? "selected" : ""}>${harian.data.staff_produksi.nama}</option>`).join("");
            } else {
