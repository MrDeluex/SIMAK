    ...
    <form action="" class="w-full" id="inputForm">
        <div class="w-full p-10 flex flex-col justify-start items-start gap-8"
            ...
            <div class="w-full flex justify-between items-center">
                <button type="button" class="px-10 py-1 rounded bg-button-false text-white"  onclick="window.location.href='/admin/harian'">Kembali</button>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", async function() {
            ...
            async function loadBarangProduksi() {
                try {
                    const res = await fetch("https://backend-simak.trihech.my.id/api/admin/barang", {
                        headers: {
                            "Authorization": "Bearer {{ session('api_token') }}"
                        }
                    });
            ...
            form.addEventListener("submit", async function(event) {
                ...
                try {
                    const res = await fetch("https://backend-simak.trihech.my.id/api/admin/barang-harian", {
                        method: "POST",
                        headers: {
